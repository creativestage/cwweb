...
[点击查看配置项文档](https://github.com/creativestage/cwweb/wiki/%E9%85%8D%E7%BD%AE%E9%A1%B9)
...
js脚本最外层上下文指向了模块通用配置对象 \`Compiler\`，属性和方法如下：

* el: \`String\` 模块id，透传给vue; 如：\`new Vue({}).$mount(this.el)\`;
* config: \`Object\` 配置项最终的输入的值；
* Event: \`Object\` 自定义全局事件对象
  * on: \`Function\` 事件注册，如：\`this.Event.on('Form:SubmitSuccess', () => {console.log('success')})\`;
  * emit: \`Function\` 事件触发，如：\`this.Event.emit('Form:SubmitSuccess')\`
* require: \`Function\` 加载js文件，如： \`this.require('http://xxx').then(() => {console.log('loaded')})\`
